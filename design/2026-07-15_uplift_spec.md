...
`oracle/` harness performed 2026-07-15.
...
| F9: README overstates ("validated against the authors' reference implementation") | Yes, README vs `oracle/aci_oracle_dyad.m` header | Confirmed: the oracle is an independent harness reproducing the deterministic core of `marandmath/ACI_code`, not an unmodified upstream run |
...
5. **Oracle provenance manifest** (F9): `inst/extdata/oracle-manifest.yml`
   ...
   (`oracle/refresh_fixtures.R` guidance) that regenerates to a temp dir,
