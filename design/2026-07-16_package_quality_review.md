...
| Root `oracle/`, `matlab_reference/` | MATLAB harnesses and authors’ reference |
...
Local probe (source load via `devtools::load_all`): flagship dyad path, diagnostic summary, correlated-noise simulation rejection, and wall time for `n = 30001` (median 0.036 s over five runs on `aarch64-apple-darwin20`, R 4.5.2; artefact: `design/artefacts/2026-07-16_probe.txt`).
...
| PQ-T4 | Info | Coverage measured at 100.00% (`covr::package_coverage()`; artefact: `design/artefacts/2026-07-16_coverage.txt`); the *important* fact is that ungraded *paths* (cross-noise) were fixed by new oracles rather than by chasing line coverage. Keep that discipline. |
