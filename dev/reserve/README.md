...
`reserve/` is `.Rbuildignore`d (`^reserve($|/)`), so it costs the built
package nothing — verified: the 0.1.0 tarball's top level is
`DESCRIPTION LICENSE NAMESPACE R inst man tests`.
...
git apply reserve/fbcir/fbcir.patch
R CMD INSTALL . --library=<a scratch library>
```

Both were last regenerated against the current mainline; the figures below are
from that regeneration, and each family's `NOTES.md` carries the full log.

| | applies to clean `main` | installs | test suite |
|---|---|---|---|
| `fbcir.patch` (1031 lines) | clean (`git apply --check`) | `* DONE (acir)` | **6397 pass, 0 fail, 0 error, 1 skip** |
| `enkbs.patch` (2610 lines) | clean (`git apply --check`) | `* DONE (acir)` | **6483 pass, 0 fail, 0 error, 1 skip** |

Mainline `main` is 6236 pass / 0 fail, so the two patches reinstate 161 and
247 assertions respectively. The single skip in every run is the optional
