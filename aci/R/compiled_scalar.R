################################################################################
## compiled_scalar.R - private compiled-CGNS contract and scalar proof kernels
################################################################################
...
.new_compiled_cgns <- function(rs, source_model, source_obs, coefficients,
  ...
  out <- list(
    ...
    provenance = list(source_model = source_model, source_obs = source_obs,
                      model = m, obs = ob, nontarget = rs$tag)
  )
  if (k == 1L && l == 1L) {
    out$scalar <- list(
...
#' the deterministic one-pass generic compiler.
#'
#' @param model A `cgns_model`.
#' @param obs Anything accepted by [as_obs()].
#' @param nontarget Optional non-target specification.
#' @returns A compiled CGNS bundle.
#' @noRd
.compile_cgns_run <- function(model, obs, nontarget = NULL) {
  descriptor <- if (inherits(model, "cgns_model"))
    .cgns_realizer_descriptor(model) else NULL
  if (is.null(nontarget) && !is.null(descriptor) &&
      identical(descriptor$id, "dyad_observed_x_v1"))
    return(.compile_dyad_cgns(model, obs, nontarget = NULL))
  if (!is.null(descriptor) &&
      identical(descriptor$id, "affine_model_v1"))
    return(.compile_affine_cgns(model, obs, nontarget = nontarget))
  .compile_cgns_complete(model, obs, nontarget = nontarget)
}
...
#'
#' @param model An unmodified `model_dyad(observe = "x")` model.
#' @param obs Anything accepted by `as_obs()`.
#' @param nontarget Must be `NULL` for this one-observation directed realiser.
#' @returns A private `compiled_cgns` object with the same contract as the generic
#'   closure fallback.
#' @noRd
.compile_dyad_cgns <- function(model, obs, nontarget = NULL) {
  descriptor <- .cgns_realizer_descriptor(model)
  if (!inherits(model, "cgns_model") || model$k != 1L || model$l != 1L ||
      !is.null(nontarget) || is.null(descriptor) ||
      !identical(descriptor$id, "dyad_observed_x_v1") ||
      !is.list(descriptor$spec$params))
    aci_abort("aci_error_compiled_contract",
              "The directed dyad realiser requires an unconditioned observed-x model_dyad().")
...
#' @param nontarget Optional expected non-target specification.
...
.validate_compiled_cgns <- function(bundle, model = NULL, obs = NULL,
  ...
  if (!identical(bundle$contract_version, 1L) ||
      ...
      !identical(bundle$nontarget %||% NULL,
                 bundle$provenance$nontarget %||% NULL) ||
  ...
  if (!identical(bundle$nontarget %||% NULL, nontarget %||% NULL))
  ...
  if (nsub == 1L) {
    for (j in seq_len(N)) {
      ...
      R <- R + (Ly[j] * R + R * Ly[j] + gyy[j] -
        Kg * (Lx[j] * R + gyx[j])) * h
      if (!is.finite(R)) .scalar_spd_floor(R)
      if (R <= 0) R <- max(1e-12 * max(abs(R), 1e-300), 1e-300)
...
#' @noRd
.cgns_likelihood_scalar_kernel <- function(bundle, filter_mean, filter_cov) {
  s <- bundle$scalar; dt <- bundle$dt; ll <- 0
  for (j in seq_len(bundle$N)) {
    iota0 <- (s$rate[j] - (s$fx[j] + s$Lx[j] * filter_mean[j])) * dt
    Sd <- (s$Lx[j]^2 * filter_cov[j] * dt + s$gxx[j]) * dt
    if (!is.finite(Sd))
      aci_abort("aci_error_spd",
                "spd_floor() needs a finite non-empty square matrix and eps > 0.")
    if (Sd <= 0) Sd <- .scalar_spd_floor(Sd)
    ch <- sqrt(Sd)
    w <- iota0 / ch
...
.cgns_filter_scalar <- function(bundle, init = NULL, nsub = 1L,
                                validate = TRUE) {
  if (isTRUE(validate))
    .validate_compiled_cgns(bundle, nontarget = bundle$nontarget, scalar = TRUE)
  ...
  out <- .cgns_filter_scalar_kernel(bundle, ini$mu, ini$R, nsub)
  ll <- .cgns_likelihood_scalar_kernel(bundle, out$mean, out$cov)
  if (out$stability > 1)
...
.cgns_smoother_scalar_kernel <- function(bundle, filter_mean, filter_cov,
  ...
  if (nsub == 1L) {
    for (j in (N1 - 1L):1L) {
      ...
      if (!is.finite(Rs)) .scalar_spd_floor(Rs)
      if (Rs <= 0) Rs <- max(1e-12 * max(abs(Rs), 1e-300), 1e-300)
      MU[j] <- mus; CV[j] <- Rs
    }
  } else {
    for (j in (N1 - 1L):1L) {
...
.cgns_smoother_scalar <- function(bundle, filter, validate = TRUE) {
  if (isTRUE(validate)) {
    ...
    .validate_gaussian_path(filter, bundle$obs, 1L, "filter",
                            bundle$nontarget, model = bundle$model,
  ...
  out <- .cgns_smoother_scalar_kernel(bundle, fm, fc, nsub)
  MU <- matrix(out$mean, bundle$N1, 1L)
  CV <- array(out$cov, c(1L, 1L, bundle$N1))
...
.gaussian_kl_scalar_kernel <- function(mu_p, R_p, mu_q, R_q,
  ...
  Lq <- sqrt(R_q); Lp <- sqrt(R_p)
  signal <- 0.5 * ((mu_q - mu_p) / Lq)^2
  A <- Lp / Lq
  dispersion <- 0.5 * (A * A - 1 + 2 * log(Lq) - 2 * log(Lp))
...
#' @returns A data frame shaped like `gaussian_kl_path()`.
#' @noRd
.gaussian_kl_path_scalar <- function(bundle, p, q, decompose = TRUE,
  ...
  if (length(p$t) != n || length(q$t) != n ||
    ...
    aci_abort("aci_error_dims",
              "gaussian_kl_path: incompatible or non-finite moments.")
...
.aci_scalar_compiled <- function(bundle, init = NULL, nsub = 1L,
  ...
  structure(list(
    ...
    handles = list(model = bundle$source_model, obs = bundle$source_obs,
                   nontarget = bundle$nontarget, init = filt$meta$init),
    meta = list(engine = "cgns", nontarget = bundle$nontarget, m = NULL,
