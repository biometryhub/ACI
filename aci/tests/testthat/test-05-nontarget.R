chain_model <- function(gA = 0) cgns_model(
...
test_that("T7a: chain y -> xB -> xA gives near-zero conditional ACI", {
  ...
  a_c  <- suppressWarnings(aci(m, s$obs, nontarget = nontarget(blocks = 2)))
  burn <- seq_len(200)
  expect_gt(max(a_un$aci[-burn]), 20 * max(a_c$aci[-burn]))
})
test_that("T7b: 'prescribed_forcing' and 'inflate' agree when both defined", {
  m <- chain_model(gA = 1)
  s <- simulate(m, seed = 34, T = 3, dt = 2e-3, ic = list(x0 = c(0, 0), y0 = 0.5))
  a1 <- suppressWarnings(aci(m, s$obs,
          nontarget = nontarget(blocks = 2, strategy = "prescribed_forcing")))
  a2 <- suppressWarnings(aci(m, s$obs,
          nontarget = nontarget(blocks = 2, strategy = "inflate")))
...
test_that("prescribed-forcing paths retain stable source-model provenance", {
  ...
  nt <- nontarget(blocks = 2, strategy = "prescribed_forcing")
  f <- suppressWarnings(da_filter(m, s$obs, nontarget = nt))
  sm <- expect_no_error(da_smooth(m, s$obs, filter = f, nontarget = nt))
  expect_s3_class(sm, "da_path_gaussian")
  expect_no_error(lag_table(m, s$obs, mode = "one_lag", filter = f,
                            nontarget = nt))
  expect_s3_class(suppressWarnings(aci(m, s$obs, nontarget = nt)),
                  "aci_result")
})
...
test_that("non-target block specifications reject empty, fractional, and duplicate indices", {
  ...
  for (bad in list(numeric(0), 1.5, c(1, 1), 3))
    expect_error(da_filter(m, s$obs, nontarget = nontarget(bad, "inflate")),
...
test_that("masked-innovation likelihood scores only target components", {
  ...
  nt <- nontarget(2, "inflate")
  ini <- list(mean = 0, cov = matrix(1, 1, 1))
  f1 <- da_filter(m, ob1, nontarget = nt, init = ini)
  f2 <- da_filter(m, ob2, nontarget = nt, init = ini)
