...
test_that("directed dyad realiser fills the same neutral compiled contract", {
  ...
  expect_error(.compile_dyad_cgns(
    ds$model, ds$obs, nontarget = nontarget(1, "inflate")),
    class = "aci_error_compiled_contract")
})
...
test_that("compiled identity excludes init but binds model, observations, and policy", {
  ...
  expect_error(.validate_compiled_cgns(
    ds$bundle, nontarget = nontarget(1, "inflate")),
...
test_that("compiled bundles reject internal mutation and mismatched metric grids", {
  ...
  for (mutate in mutations)
    expect_error(.validate_compiled_cgns(
      mutate(ds$bundle), nontarget = NULL, scalar = TRUE),
      class = "aci_error_compiled_contract")
...
test_that("prescribed forcing compiles after resolution to a scalar contract", {
  ...
  bundle <- .compile_cgns_complete(model, sim$obs, nontarget = nt)
  public <- da_filter(model, sim$obs, init = init, nontarget = nt)
  ...
  expect_identical(bundle$nontarget, nt)
...
test_that("private scalar kernels and production public routing agree", {
  ...
  for (nsub in c(1L, 3L)) {
    public_filter <- da_filter(ds$model, ds$obs, init = ds$init,
    ...
    public_smoother <- da_smooth(ds$model, ds$obs, filter = public_filter)
    ...
    for (decompose in c(FALSE, TRUE)) {
      public_metric <- gaussian_kl_path(public_smoother, public_filter,
                                        decompose = decompose)
...
test_that("scalar proof preserves warnings and covariance policy", {
  ...
  public_default <- capture_aci_warnings(da_filter(ds$model, ds$obs))
  ...
  public_stiff <- capture_aci_warnings(
    da_filter(stiff$model, stiff$obs, init = big))
  scalar_stiff <- capture_aci_warnings(
    .cgns_filter_scalar(stiff$bundle, big))
  ...
  dt_model <- model_dyad(params = within(ds$model$meta$params, d_y <- 25))
  ...
  public_dt <- capture_aci_warnings(
    da_filter(dt_model, dt_obs, init = dt_init))
  ...
  public_floor <- capture_aci_warnings(
    da_filter(floor_model, floor_obs, init = floor_init))
  scalar_floor <- capture_aci_warnings(
    .cgns_filter_scalar(floor_bundle, floor_init))
...
test_that("time-varying correlated scalar coefficients retain route and parity", {
  ...
  bundle <- .compile_cgns_complete(model, obs)
  public_filter <- da_filter(model, obs, init = init, nsub = 2)
  scalar_filter <- .cgns_filter_scalar(bundle, init, nsub = 2)
  public_smoother <- da_smooth(model, obs, filter = public_filter)
...
test_that("terminal-only cross-noise is retained in scalar route metadata", {
  ...
  public <- da_smooth(model, obs, filter = da_filter(model, obs, init = init))
...
external_scalar_model <- function(name) {
  if (identical(name, "dyad_reference_head")) return(model_dyad())
  cgns_model(
