% ACI_ORACLE_CIR_CROSS  Causal-influence-range fixture WITH correlated noise.
%
% Written to close a gap the package's own grading register measured: the
% online smoother and the causal influence range were graded only by a fixture
% that sets the noise cross-covariance to ZERO, so the terms carrying it
% executed on every run and were checked against nothing. That is the same
% defect the `cross` fixture closed for the filter and the smoother, one layer
% up.
%
...
raw = readmatrix('../aciR/inst/extdata/cross_signal_x.csv');
M = 2001;
x = raw(1:M, 2).';
N = M - 1;
...
writetable(T1, 'cir_cross_online_reference.csv');
...
writetable(T2, 'cir_cross_range_reference.csv');

fprintf('CIR_CROSS_DONE M=%d S_yoS_x=%.4f rows1=%d rows2=%d\n', M, S_yoS_x, height(T1), height(T2));
