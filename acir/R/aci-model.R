################################################################################
## aci-model.R - model classes and simulation
## ########################################################################## ##
##
## Contents:
...
##         aci_linear_model, aci_model_from_affine
##
...
#' model that `cgns_model` inherits from. The observed and hidden drifts and
#' diffusions are supplied as functions and validated on a small probe set
#' before the object is returned. Not exported in this release: the general
#' class exists so that `cgns_model` dispatch and the class string are
#' preserved.
#'
...
print.stochastic_model <- function(x, ...) {
  ...
  if (isTRUE(x$meta$matlab_defect_compat$active))
    cat(sprintf("  MATLAB defect compatibility: %s\n",
                paste(names(x$meta$matlab_defect_compat$defects),
                      collapse = ", ")))
  invisible(x)
}
...
aci_model <- function(Lx, fx, Ly, fy, Sx1, Sx2 = NULL, Sy1 = NULL, Sy2,
  ...
  # channels W1, W2): Sx2 pairs with Sy2, Sy1 pairs with Sx1.  The width is
  # resolved on the first call and kept: shared-channel counts are required to
  # be constant over state and time (checked below in validate_cgns(), and
  # again at every probe against the partner the width was taken from), so
  # re-evaluating the partner on every step only to read its column count is
  # work that the realisers pay for on the whole observation grid.
  if (is.null(Sx2)) Sx2 <- local({
    width <- NULL
    function(t, x) {
      if (is.null(width)) width <<- ncol(as.matrix(Sy2(t, x)))
      matrix(0, k, width)
    }
  })
  if (is.null(Sy1)) Sy1 <- local({
    width <- NULL
    function(t, x) {
      if (is.null(width)) width <<- ncol(as.matrix(Sx1(t, x)))
      matrix(0, l, width)
    }
  })
...
#'
#' @export
aci_linear_model <- function(lambda_x, lambda_y, fx, fy,
  ...
  class(m) <- c("aci_linear_model", class(m))
  m
}
...
#'
#' @export
aci_model_from_affine <- function(f_full, g_full, Sx, Sy_hidden, k, l,
                                  name = NULL, Sx2 = NULL, Sy_shared = NULL,
                                  meta = list()) {
  dims <- .state_dims(k, l); k <- dims[["k"]]; l <- dims[["l"]]
...
simulate.stochastic_model <- function(object, nsim = 1, seed = NULL,
  ...
  # advance its linearised hidden drift and return a plausible-looking path of
  # a system nobody wrote down.
  if (isFALSE(object$meta$simulate_supported))
    aci_abort("aci_error_model_contract", sprintf(paste(
      "'%s' is an inference-only model: its coefficients approximate a larger",
      "system that it cannot itself generate. %sSimulate the full system with",
      "%s and build this model from that path."),
      object$name,
      if (is.null(object$meta$approximation_note)) ""
      else paste0(object$meta$approximation_note, " "),
      object$meta$simulator_model %||% "the full-system constructor"))
  ...
  ## Seeding is contained, and every argument this frame validates has been
  ## checked above, so a rejected call never reaches the generator. `set.seed()`
  ## still governs the draw stream, so seeded paths are unchanged; only the
  ## caller's state afterwards is. Materialise `.Random.seed` first when it is
  ## absent, so there is a state to put back.
  if (!is.null(seed)) {
...
#'
#' The named verb for the package's [simulate.stochastic_model()] method. It
#' exists so that the public surface reads as one family of `aci_*` verbs. It
#' is the same call, with the same arguments and the same draws, and the S3
#' method stays registered on the `stats::simulate()` generic, so
...
#'
#' @seealso [simulate.stochastic_model()], [aci_model()]
#'
#' @examples
#' m <- aci_dyad_model()
...
aci_simulate <- function(object, ...) {
  if (!inherits(object, "stochastic_model"))
    aci_abort("aci_error_model_contract",
              "aci_simulate() requires a model object.")
  return(stats::simulate(object, ...))
}


#' Observed-channel names a model declares, when they fit (internal)
#'
#' The built-in constructors record the variable names of the system they
#' realise in `meta$vars`. Carrying the observed half onto a simulated
#' trajectory is what makes `aci_conditional(target = "TC")` work on a
#' `simulate()` result, since block names resolve against `colnames(obs$x)`.
#' Anything that is not one unique non-empty name per observed channel is
#' ignored rather than repaired: `observed_trajectory()` is the contract, and a
#' model with unusable `meta$vars` simply gets unnamed columns as before.
#'
#' @param m A `stochastic_model` or `cgns_model` object.
#' @returns A character vector of length `m$k`, or `NULL`.
#' @noRd
.model_obs_names <- function(m) {
  nm <- m$meta$vars$observed
  if (!is.character(nm) || length(nm) != m$k || anyNA(nm) ||
