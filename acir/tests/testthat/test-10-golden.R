...
test_that("G1: P1 golden filter, smoother, and ACI agree to machine precision", {
  ...
  fp <- aci_filter(gs$m, gs$s$obs, init = gs$ini)
  sp <- aci_smoother(gs$m, gs$s$obs, filter = fp)
  ...
  kl <- t(vapply(j, function(i)
    aci_metric_pair(g$sm[i], matrix(g$sc[i], 1, 1),
...
test_that("G2: golden CIR definition matches exact; ratio differs only by quadrature", {
  ...
  expect_warning(fc_ <- aci_range(tab), class = "aci_warn_low_signal")
  expect_warning(l1 <- aci_range(tab, method = "l1_linf"),
  ...
  # Ledger entry C2b: the golden's defn_objective_CIR is a Simpson quadrature
  # of the subjective ranges over its 513-point threshold grid.  That route is
  # now a named mode of its own and is graded against the golden directly.
  compat <- suppressWarnings(
    aci_range(tab, method = "exact", quadrature = "matlab_eps_grid"))
  rel_compat <- abs(compat$tau[ok] - gc0$defn_obj[ok]) /
                pmax(gc0$defn_obj[ok], 1e-12)
  expect_lt(max(rel_compat), 1e-12)                  # measured 3.43e-14
  expect_identical(compat$bound, "eps_grid_objective")
  expect_equal(compat$meta$epsilon_grid, 10^seq(-6, 0.5, length.out = 513L))
  # The package default evaluates the same threshold average exactly, so it
  # does not reproduce the golden's grid error.  The residual below is that
  # grid error, measured, and it is what the compat mode exists to reproduce.
  rel_defn <- abs(fc_$tau[ok] - gc0$defn_obj[ok]) /
              pmax(gc0$defn_obj[ok], 1e-12)
  expect_lt(stats::median(rel_defn), 0.015)          # measured 0.00424, and 0.00703 before this entry
  ...
  l1_trap <- suppressWarnings(
    aci_range(tab, method = "l1_linf", simpson_close = "trapezoid"))
