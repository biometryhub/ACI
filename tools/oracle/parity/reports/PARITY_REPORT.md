...
Generated from `tools/oracle/parity/`, 2026-08-14, extended 2026-08-16. Scope: the
...
hand transcription in `tools/oracle/aci_oracle_dyad.m`. That transcription and the R
...
over 12,000 steps. This is benign, and the informative part is the scale. Had the
...
reference. It is a random-walk observation with trigonometric components and, most
...
The reference values are the captured workspaces' own, taken from the composed
runner, since this script must not be run top to bottom (F6), so nothing was
...
`cir_objective_exact`, the reference's `defn_objective_CIR` and F2's quantity,
...
reference-conventions row, so the design difference is below round-off. On
...
how much live tail a horizon removes, rather than a standing bias, which is the
...
| `objective` | 4.27e-03 | n/a |
| `subjective` | 1.16e+00 | n/a |
...
truncated number anyway. On this evidence aciR's behaviour is the better of the
two, since the reference's value there is an artefact of where its matrix
happened to end.

**The part that matters for the package.** `tools/oracle/aci_oracle_cir.m:162` loops
...
by the quadratic through the last three samples integrated over the last
interval. The rule is derived rather than transcribed, and asserted to reduce to
...
`max(numeric(0))` is `-Inf`, which passes any threshold. A comparison in
...
This is not a defect when the script is used as documented, since its own banners
...
latent variable in a script whose latent variable is tau. This is harmless, since the
...
one. See F9 for the interface difference that has to be handled before
...
That is a reasonable choice and arguably the safer one, but it is a **behavioural
divergence**, not an implementation detail: a system the reference will analyse,
aciR will decline. Documented, not changed, because silently pseudo-inverting a
...
pseudoinverse fills slice one, by `S_xoS_x_inv(:, :, 1) = pinv(S_x(:,:,1) *
...
one included. That is the stronger reading of the construction, since the
whole point is to decline to condition on the other channels, and it is not
...
state ever takes**. It is a transient, since the filter forgets its initial
condition, and it is a slow one. Some 6,000 of 20,001 steps pass before it falls
...
the target and up to 3076.9, which is `1/sigma_E^2`, on a channel the
...
- The reference parameterises noise by feedback matrices and forms the
  ...
  one, since not every admissible Grammian pair is two feedback columns of that
  shape.
...
The machinery is general. This section records how much of the reference has
been carried through it.
...
contract, which is scalar-only today. It also cannot be done as a fixture
re-grade, because `tools/oracle/aci_oracle_enso.m` deliberately generates its signal
...
wording was right, in that each script needs its own input list, and the
...
script.** The reference side is complete at G1 32/32, and `aci_conditional()`
...
1. The vector dataset contract in `tools/dataset_mv.R` assumes a square
   ...
   complaint, since `L_x` is `n_x` by `n_y` and `L_y` is `n_y` by `n_y`, so the work
...
harness now grades on the reference's own conventions, namely `horizon =
last_idx`, `margin` stood down and the reference's threshold grid, and reports aciR's
...
* Every surface that still said the online smoother and CIR were "scalar-only"
  (`DESCRIPTION`, `README`, the assumptions vignette and two oracle-manifest
...
Declined, with reasons, in `tools/design/2026-08-14_reviewer_OUTPUT.md` order: S7
(parallel CIR at 0.082 s, and `mclapply` is Unix-only), S3 (`pinv` escape, where
...
manifests in `tools/oracle/parity/`, and neither is a defect when the scripts are
...
being the same observation about the same files. The count in it was wrong, so
