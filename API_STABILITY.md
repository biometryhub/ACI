...
- The numerical core is fixed. Every graded quantity reproduces the
  comparator its row names in `inst/evidence/register.csv` (the method
  authors’ MATLAB outputs where such outputs exist, and otherwise a
  source-derived run, an independent transcription of the published
  equations, or an analytic identity) to that row’s tolerance, and a
  change that moved a number beyond round-off would be a change of
  method, not a release.
